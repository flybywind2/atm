...
async def start_analysis(
    ...
    # CRITICAL DEBUG: Force log API request reception
    logger.info(f"===== START_ANALYSIS API CALLED =====")
    logger.info(f"Thread ID: {thread_id}")
    logger.info(f"Problem: {request.problem_description[:100]}...")
    print(f"[FORCE DEBUG] START_ANALYSIS API CALLED - Thread: {thread_id}")
    ...
    try:
        ...
        # Run workflow asynchronously so the frontend can poll progress
...
async def run_langgraph_workflow(thread_id: str, initial_state: ProblemSolvingState):
    ...
    try:
        ...
        async with AsyncSqliteSaver.from_conn_string(":memory:") as checkpointer:
            ...
            try:
                async for output in compiled_workflow.astream(
                    ...
                    # Add delay to allow frontend to catch intermediate states
                    import asyncio
                    await asyncio.sleep(2.0)  # 2 second delay between steps for visibility
        ...
        # IMPROVED: Gradual progress instead of sudden jump to 100%
        current_progress = workflow_data.get("progress_percentage", 20)

        # Step 1: Switch to fallback mode (50% progress)
        workflow_data["status"] = WorkflowStatus.PROCESSING
        workflow_data["progress_percentage"] = 50
        workflow_data["message"] = "기본 워크플로우 실패, 폴백 문서 생성 중..."
        workflow_data["current_step"] = "fallback_generation"
        workflow_data["updated_at"] = datetime.utcnow().isoformat()

        # Step 2: Generate all 4 required documents as fallback (80% progress)
        workflow_data["progress_percentage"] = 80
        workflow_data["message"] = "포괄적인 폴백 문서 생성 중..."
        fallback_results = {
        ...
        # Step 3: Final completion (100% progress)
        workflow_data["status"] = WorkflowStatus.COMPLETED
        workflow_data["progress_percentage"] = 100
        workflow_data["message"] = "분석이 폴백 문서 생성을 통해 완료되었습니다 (Fallback으로 생성됨)"
        workflow_data["updated_at"] = datetime.utcnow().isoformat()
...
🤖 **폴백으로 생성된 문서**: 실제 요구사항에 맞게 수정해주세요.
"""
...
🤖 **폴백으로 생성된 문서**: 실제 비즈니스 로직에 맞게 수정해주세요.
"""
...
🤖 **폴백으로 생성된 문서**: 실제 요구사항에 맞게 조정해주세요.
"""
...
async def update_workflow_progress(thread_id: str, state_output: Dict[str, Any]):
    ...
    # Map workflow steps to progress percentages (monotonic)
    step_progress = {
        "analyze_problem": 20,
        "problem_analyzed": 25,
        "collect_context": 40,
        "collecting_context": 50,  # waiting for additional input
        "context_collected": 45,
        "generate_requirements": 60,
        "requirements_generated": 65,
        "design_solution": 80,
        "solution_designed": 85,
        "create_guide": 90,
        "guide_created": 95,
        "completed": 100
    }
    ...
    current_progress = workflow_data.get("progress_percentage", 0)
    target_progress = step_progress.get(current_step, current_progress)
    # ensure progress never decreases
    progress = max(current_progress, target_progress)

    # DEBUG: Log progress updates
    logger.info(f"[PROGRESS UPDATE] Thread {thread_id}: Step='{current_step}', Progress={progress}%")
    print(f"[FORCE DEBUG] UPDATE_WORKFLOW_PROGRESS CALLED - Thread: {thread_id}, Step: {current_step}, Progress: {progress}%")

    # CRITICAL FIX: Update results in real-time as documents are generated
    current_results = workflow_data.get("results", {})

    # Check for newly generated documents and add them to results immediately
    if state_output.get("requirements_document"):
        current_results["requirements_document"] = state_output["requirements_document"]
        logger.info(f"[REAL-TIME] Added requirements_document to results for {thread_id}")

    if state_output.get("user_journey_map"):
        current_results["user_journey_map"] = state_output["user_journey_map"]
        logger.info(f"[REAL-TIME] Added user_journey_map to results for {thread_id}")

    if state_output.get("implementation_guide"):
        current_results["implementation_guide"] = state_output["implementation_guide"]
        logger.info(f"[REAL-TIME] Added implementation_guide to results for {thread_id}")

    if state_output.get("tech_recommendations"):
        current_results["tech_recommendations"] = state_output["tech_recommendations"]
        logger.info(f"[REAL-TIME] Added tech_recommendations to results for {thread_id}")

    if state_output.get("solution_type"):
        current_results["solution_type"] = state_output["solution_type"]
        logger.info(f"[REAL-TIME] Added solution_type to results for {thread_id}")

    if state_output.get("technology_stack"):
        current_results["tech_stack"] = state_output["technology_stack"]
        logger.info(f"[REAL-TIME] Added tech_stack to results for {thread_id}")

    # Heuristic: bump progress if certain artifacts are present (in case current_step is missing)
    if state_output.get("requirements_document"):
    ...
    workflow_data["results"] = current_results  # Store real-time results
    
    # Update status based on current step
    if current_step in ["analyze_problem"]:
        workflow_data["status"] = WorkflowStatus.ANALYZING
        workflow_data["message"] = "문제 구조 분석 중..."
    elif current_step in ["problem_analyzed"]:
        workflow_data["status"] = WorkflowStatus.ANALYZING
        workflow_data["message"] = "문제 분석 완료, 컨텍스트 수집 중..."
    elif current_step in ["collect_context"]:
        workflow_data["status"] = WorkflowStatus.COLLECTING_CONTEXT
        workflow_data["message"] = "추가 컨텍스트 수집 중..."
    elif current_step in ["context_collected"]:
        workflow_data["status"] = WorkflowStatus.COLLECTING_CONTEXT
        workflow_data["message"] = "컨텍스트 수집 완료, 요구사항 생성 중..."
    elif current_step in ["generate_requirements"]:
        ...
        workflow_data["message"] = "구현 가이드 생성 중..."
    elif current_step in ["guide_created"]:
        workflow_data["status"] = WorkflowStatus.CREATING_GUIDE
        workflow_data["message"] = "구현 가이드 생성 완료..."
    elif current_step == "completed":
        workflow_data["status"] = WorkflowStatus.COMPLETED
        workflow_data["message"] = "분석 완료!"
    ...
    logger.info(f"[STATUS UPDATE] Thread {thread_id}: Status={workflow_data['status']}, Message='{workflow_data['message']}'")
    print(f"[FRONTEND DEBUG] Thread {thread_id} - Step: {current_step}, Progress: {progress}%, Status: {workflow_data['status']}")
...
async def start_problem_solving(request: ProblemSolvingRequest):
    ...
    workflow_data = {
        ...
        "progress_percentage": 5,  # Start at 5% to show initial activity
        "current_step": "analyze_problem",
        "message": "분석 시작 중...",
        "results": {},
        "created_at": datetime.utcnow().isoformat(),
