...
class ProgressTrackerComponent {
    ...
    updateWorkflowSteps(currentStep, status) {
        ...
        const statusToStep = {
            ...
            'generating_requirements': 'requirements',
            'designing_solution': 'solution',
            'creating_guide': 'guide',
            'processing': stepMapping[currentStep] || 'analyze',
            'completed': 'guide'
        };
        const activeStepType = statusToStep[normalizedStatus] || stepMapping[currentStep];
        ...
        stepItems.forEach(item => {
            ...
            if (activeStepType === stepType) {
                ...
                const currentIndex = stepOrder.indexOf(activeStepType);
        ...
        this.applyStepStatusEmojis(normalizedStatus);
    }

    /**
     * Apply emojis to step status based on classes
     */
    applyStepStatusEmojis(normalizedStatus) {
        const stepItems = this.container.querySelectorAll('.step-item');
        stepItems.forEach(item => {
            ...
            if (item.classList.contains('error')) {
                ...
                statusEl.textContent = (normalizedStatus === 'paused' || normalizedStatus === 'awaiting_input') ? '⏸️' : '🔄';
            } else {
                statusEl.textContent = '⏳';
            }
        });
